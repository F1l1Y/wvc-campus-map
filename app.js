...
fetch("campus.geojson?v=39").then(r => r.json()).then(gj => {
...
/* ------------------------------------------------------------- rooms */
function loadRooms() {
  return fetch("rooms.json?v=39").then(r => r.json()).then(j => {
    DATA.roomsDoc = j;
    ...
    // plans
    return Promise.all(["pe","lrc"].map(id => fetch(`plans/${id}.json?v=39`).then(r => r.json()).then(p => {
...
function loadWalk() {
  fetch("walkgraph.json?v=39").then(r => r.json()).then(j => {
    DATA.walk = j;
...
function loadCoverage() {
  fetch("coverage.json?v=39").then(r => r.json()).then(j => { DATA.coverage = j; }).catch(() => {});
}
function loadRoutes() {
  fetch("evac_routes.json?v=39").then(r => r.json()).then(j => { DATA.routes = j; }).catch(() => {});
}
function loadAmenities() {
  fetch("amenities.json?v=39").then(r => r.json()).then(j => { DATA.amen = j; buildAmenityLayers(j); });
}
...
function buildIndoor(plan) {
  ...
  const overlay = L.imageOverlay(plan.image + "?v=39", bounds, {
...
function planSVG(plan, highlight) {
  ...
  const img = raster ? `<image href="${esc(plan.image)}?v=39" x="0" y="0" width="${plan.width}"
